...
def index(request):
	return render(request,'mandiPrice/index.html')

def scrapMandiPrice(request):
	autoIncr=[0]
	import requests
	...
	def popLis(sp,lis,autoIncr):
	    ...
	    for i,tr in enumerate(tr_temp[1:]):
	        ...
	        if len(coli) == 9:
	        	...
	        	try:
	        		obj=get_object_or_404(MandiPrice,id=autoIncr[0])        	
		        	obj.date=coli[8]
		        	obj.district=coli[0]
		        	obj.market=coli[1]
		        	obj.commodity=coli[2]
		        	obj.variety=coli[3]
		        	obj.grade=coli[4]
		        	obj.minP=coli[5]
		        	obj.maxP=coli[6]
	        		...
	        		obj=MandiPrice.objects.create(id=autoIncr[0],date=coli[8],district=coli[0],market=coli[1],commodity=coli[2],variety=coli[3],grade=coli[4],minP=coli[5],maxP=coli[6],modP=coli[7])
		        	obj.save() 
	    ...
	    try:        
	        ...
	        if str(initResp).find('Page$Next') != -1:
	            ...
	            while str(nextP).find('Page$Next') != -1:
	                ...
	                popLis(soup(nextP,'lxml'),lis,autoIncr)
	        ...
	        if tries <= 100:
	            ...
	            break
	return render(request,'mandiPrice/scrapingDone.html',{})
